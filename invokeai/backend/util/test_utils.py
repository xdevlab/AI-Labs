...
def model_installer():
    ...
    # which can cause `install_and_load_model(...)` to re-download model unnecessarily. As a temporary workaround, we
    # pass a kwarg to get_config, which causes the config to be re-loaded.
